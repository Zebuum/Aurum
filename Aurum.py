...
def openDailyExpenses():
    ...
    def addDailyExpenses(supermarket, zimies, paidia, agores, orio, ora):
        conn = sqlite3.connect("aurum.db")
        c = conn.cursor()
        c.execute("INSERT INTO dailyexpenses VALUES (?,?,?,?,?,?)", (supermarket, zimies, paidia, agores, orio, ora))
        conn.commit()
        conn.close()

    def viewDailyExpenses():
        conn = sqlite3.connect("aurum.db")
        c = conn.cursor()
        c.execute("SELECT * FROM dailyexpenses")
        rows = c.fetchall()
        conn.close()
        return rows

    def deleteDailyExpenses(supermarket, zimies, paidia, agores, orio, ora):
        conn = sqlite3.connect("aurum.db")
        c = conn.cursor()
        c.execute("""DELETE FROM  dailyexpenses
    ...
    def updateDailyExpenses(supermarket, zimies, paidia, agores, orio, ora):
        conn = sqlite3.connect("aurum.db")
        c = conn.cursor()
        c.execute("""UPDATE  dailyexpenses SET
        supermarket = :supermarket,
        zimies = :zimies,
        paidia = :paidia,
        agores = :agores,
    ...
    class DailyExpenses:
            ...
            def __init__(self):
                ...
                self.root.title(200 * blank_space + "A U R U M ")
                self.root.geometry("1350x580+200+200")
                supermarket = DoubleVar()
                zimies = DoubleVar()
                paidia = DoubleVar()
                agores = DoubleVar()
                orio =DoubleVar()
                ora = DoubleVar()
                #synolo = DoubleVar(supermarket, zimies, paidia, agores)
                #synolo = supermarket.get() + zimies.get() + paidia.get() + agores.get()
                ...
                def iReset():
                    self.txtSupermarket.delete(0, END)
                    self.txtZimies.delete(0, END)
                    self.txtPaidia.delete(0, END)
                    self.txtAgores.delete(0, END)
                    self.txtOrio.delete(0, END)
                    self.txtOra.delete(0, END)
                ...
                def iAddData():
                    ...
                    if {
                        ...
                        tkinter.messagebox.showwarning("Σύστημα", "Δεν έχετε προσθέσει κάποια περιγραφή στα έξοδά σας")
                    else:
                        addDailyExpenses(supermarket.get(), zimies.get(), paidia.get(), agores.get())
                        iDisplay()

                def iDisplay():
                    result = viewDailyExpenses()
                    if len(result) != 0:
                        self.productlist.delete(*self.productlist.get_children())
                        for row in result:
                            self.productlist.insert('', END, values=row)
                ...
                def iDelete():
                    iDeleteMessage = tkinter.messagebox.askyesno("Σύστημα", "Θέλετε σίγουτα να κάνετε Διαγραφή;")
                    if iDeleteMessage > 0:
                        deleteDailyExpenses(supermarket.get(), zimies.get(), paidia.get(), agores.get())
                        tkinter.messagebox.showinfo("Σύστημα", "Επιτυχής Διαγραφή")
                        iDisplay()

                def iDailyExpenses(e):
                    iReset()
                    selected = self.productlist.focus()
                    values = self.productlist.item(selected, 'values')
                    self.txtSupermarket.insert(0, values[0])
                    self.txtZimies.insert(0, values[1])
                    self.txtPaidia.insert(0, values[2])
                    self.txtAgores.insert(0, values[3])
                    self.txtOrio.insert(0, values[4])
                    self.txtOra.insert(0, values[5])


                def iCheck():
                    if {
                        supermarket.get().isalpha(),
                        zimies.get().isalpha(),
                        paidia.get().isalphha(),
                        agores.get().isalpha(),
                        orio.get().isalpha()
                ...
                # ========================================Title==================================
                self.lblTitle = Label(TitleFrame, font=('arial', 56, 'bold'), text="Καθημερινά έξοδα", bd=7)
                self.lblTitle.grid(row=0, column=0, padx=132)
                # ========================================Widget==================================

                self.lblSupermarket = Label(LeftFrame1, font=('arial', 12, 'bold'), text="Super Market", bd=7, anchor=W,
                                     justify=LEFT)
                self.lblSupermarket.grid(row=0, column=0, sticky=W, padx=5)
                self.txtSupermarket = Entry(LeftFrame1, font=('arial', 10, 'bold'), bd=5, width=40, justify=LEFT,
                                     textvariable=supermarket)
                self.txtSupermarket.grid(row=0, column=1)

                self.lblZimies = Label(LeftFrame1, font=('arial', 12, 'bold'), text="Ζημιές", bd=7, anchor=W,
                                      justify=LEFT)
                self.lblZimies.grid(row=1, column=0, sticky=W, padx=5)
                self.txtZimies = Entry(LeftFrame1, font=('arial', 10, 'bold'), bd=5, width=40, justify=LEFT,
                                      textvariable=zimies)
                self.txtZimies.grid(row=1, column=1)

                self.lblPaidia = Label(LeftFrame1, font=('arial', 12, 'bold'), text="Έξοδα παιδιών", bd=7, anchor=W,
                                       justify=LEFT)
                self.lblPaidia.grid(row=2, column=0, sticky=W, padx=5)
                self.txtPaidia = Entry(LeftFrame1, font=('arial', 10, 'bold'), bd=5, width=40, justify=LEFT,
                                       textvariable=paidia)
                self.txtPaidia.grid(row=2, column=1)

                self.lblAgores = Label(LeftFrame1, font=('arial', 12, 'bold'), text="Λοιπές αγορές", bd=7, anchor=W,
                                       justify=LEFT)
                self.lblAgores.grid(row=3, column=0, sticky=W, padx=5)
                self.txtAgores = Entry(LeftFrame1, font=('arial', 10, 'bold'), bd=5, width=40, justify=LEFT,
                                       textvariable=agores)
                self.txtAgores.grid(row=3, column=1)

                self.lblOrio = Label(LeftFrame1, font=('arial', 12, 'bold'), text="Όριο", bd=7, anchor=W,
                                       justify=LEFT)
                self.lblOrio.grid(row=4, column=0, sticky=W, padx=5)
                self.txtOrio = Entry(LeftFrame1, font=('arial', 10, 'bold'), bd=5, width=40, justify=LEFT,
                                       textvariable=orio)
                self.txtOrio.grid(row=4, column=1)

                self.lblOra = Label(LeftFrame1, font=('arial', 12, 'bold'), text="Ώρα", bd=7, anchor=W,
                                       justify=LEFT)
                self.lblOra.grid(row=5, column=0, sticky=W, padx=5)
                self.txtOra = Entry(LeftFrame1, font=('arial', 10, 'bold'), bd=5, width=40, justify=LEFT,
                                       textvariable=ora)
                self.txtOra.grid(row=5, column=1)
                ...
                self.productlist = ttk.Treeview(RightFrame1a, height=12, columns=("supermarket", "zimies", "paidia", "agores", "orio", "ora"),
                ...
                self.productlist.heading("#0", text="")
                self.productlist.heading("supermarket", text="Super Market")
                self.productlist.heading("zimies", text="Ζημιές")
                self.productlist.heading("paidia", text="Έξοδα παιδιών")
                self.productlist.heading("agores", text="Λοιπές αγορές")
                self.productlist.heading("orio", text="Όριο")
                self.productlist.heading("ora", text="Ώρα")


                self.productlist['show'] = 'headings'

                self.productlist.column("#0", width=0)
                self.productlist.column("supermarket", width=200)
                self.productlist.column("zimies", width=200)
                self.productlist.column("paidia", width=200)
                self.productlist.column("agores", width=200)
                self.productlist.column("orio", width=200)
                self.productlist.column("ora", width=200)

                self.productlist.pack(fill=BOTH, expand=1)

                self.productlist.bind("<ButtonRelease-1>", iDailyExpenses)
                iDisplay()
                # ========================================Buttons==================================
                self.btnAddNew = Button(TopFrame1, pady=1, bd=4, font=('arial', 20, 'bold'), text="Προσθήκη", padx=24,
                                        width=12, height=2, command=iAddData).grid(row=0, column=0, padx=1)

                self.btnDisplay = Button(TopFrame1, pady=1, bd=4, font=('arial', 20, 'bold'), text="Καθαρισμός",
                                         padx=24, width=12, height=2, command=iReset).grid(row=0, column=1, padx=1)

                #self.btnReset = Button(TopFrame1, pady=1, bd=4, font=('arial', 20, 'bold'), text="Αλλαγή", padx=24,
                                       #width=12, height=2, command=iUpdate).grid(row=0, column=2, padx=1)

                self.btnDelete = Button(TopFrame1, pady=1, bd=4, font=('arial', 20, 'bold'), text="Διαγραφή", padx=24,
                                        width=12, height=2, command=iDelete).grid(row=0, column=3, padx=1)

                self.btnExit = Button(TopFrame1, pady=1, bd=4, font=('arial', 20, 'bold'), text="Έξοδος", padx=24,
                                      width=12, height=2, command=self.root.destroy).grid(row=0, column=4, padx=1)
...
def openSalary():
    ...
    def addSalary(name, price):
        conn = sqlite3.connect("aurum.db")
        c = conn.cursor()
        c.execute("INSERT INTO salary VALUES (?,?)", (name, price))
        conn.commit()
        conn.close()
    ...
    def updateSalary(name, price):
        conn = sqlite3.connect("aurum.db")
        c = conn.cursor()
        c.execute("""UPDATE  salary SET
        name = :name,
        price = :price
        WHERE name = :name """,
                  {
    ...
    class Salary:
        ...
        def __init__(self):
            ...
            def iAddData():
                iCheck()
                if name.get() == "":
                    tkinter.messagebox.showwarning("Σύστημα", "Δεν έχετε προσθέσει κάποια περιγραφή")
                else:
                    addSalary(name.get(), price.get())
                    iDisplay()
            ...
            def iUpdate():
                ...
                if name.get() == "":
                    ...
                    updateSalary(name.get(), price.get())
                    iDisplay()
            ...
            # ========================================Title==================================
            self.lblTitle = Label(TitleFrame, font=('arial', 56, 'bold'), text="Συνολικά χρήματα", bd=7)
            self.lblTitle.grid(row=0, column=0, padx=132)
            # ========================================Widget==================================

            self.lblName = Label(LeftFrame1, font=('arial', 12, 'bold'), text="Περιγραφή", bd=7, anchor=W, justify=LEFT)
            self.lblName.grid(row=0, column=0, sticky=W, padx=5)
            self.txtName = Entry(LeftFrame1, font=('arial', 10, 'bold'), bd=5, width=40, justify=LEFT,
                                 textvariable=name)
            self.txtName.grid(row=0, column=1)

            self.lblPrice = Label(LeftFrame1, font=('arial', 12, 'bold'), text="Ποσό €", bd=7, anchor=W, justify=LEFT)
            self.lblPrice.grid(row=1, column=0, sticky=W, padx=5)
            self.txtPrice = Entry(LeftFrame1, font=('arial', 10, 'bold'), bd=5, width=40, justify=LEFT,
                                  textvariable=price)
            self.txtPrice.grid(row=1, column=1)
            ...
            self.productlist = ttk.Treeview(RightFrame1a, height=12, columns=("name", "price"),
                                            xscrollcommand=scroll_x.set, yscrollcommand=scroll_y.set)
            ...
            self.productlist.bind("<ButtonRelease-1>", iSalary)
            iDisplay()
            # ========================================Buttons==================================
            self.btnAddNew = Button(TopFrame1, pady=1, bd=4, font=('arial', 20, 'bold'), text="Προσθήκη", padx=24,
                                    width=12, height=2, command=iAddData).grid(row=0, column=0, padx=1)

            self.btnDisplay = Button(TopFrame1, pady=1, bd=4, font=('arial', 20, 'bold'), text="Καθαρισμός", padx=24,
                                     width=12, height=2, command=iReset).grid(row=0, column=1, padx=1)
            ...
            self.btnExit = Button(TopFrame1, pady=1, bd=4, font=('arial', 20, 'bold'), text="Έξοδος", padx=24, width=12,
                                  height=2, command=self.root.destroy).grid(row=0, column=4, padx=1)
...
def openHouseView():
    ...
    def viewSumExpenses():
        conn = sqlite3.connect("aurum.db")
        c = conn.cursor()
        c.execute("SELECT SUM(price) FROM monthlyexpenses")
        rows = c.fetchall()
        conn.close()
        return rows
    ...
    def viewSumSalary():
        conn = sqlite3.connect("aurum.db")
        c = conn.cursor()
        c.execute("SELECT SUM(price) FROM salary")
        rows = c.fetchall()
        conn.close()
        return rows

    def viewSumSupermarket():
        conn = sqlite3.connect("aurum.db")
        c = conn.cursor()
        c.execute("SELECT SUM(supermarket) FROM dailyexpenses")
        rows = c.fetchall()
        conn.close()
        return rows
    ...
    def viewSumZimies():
        conn = sqlite3.connect("aurum.db")
        c = conn.cursor()
        c.execute("SELECT SUM(zimies) FROM dailyexpenses")
        rows = c.fetchall()
        conn.close()
        return rows
    ...
    def viewSumPaidia():
        conn = sqlite3.connect("aurum.db")
        c = conn.cursor()
        c.execute("SELECT SUM(paidia) FROM dailyexpenses")
        rows = c.fetchall()
        conn.close()
        return rows
    ...
    def viewSumAgores():
        conn = sqlite3.connect("aurum.db")
        c = conn.cursor()
        c.execute("SELECT SUM(agores) FROM dailyexpenses")
        rows = c.fetchall()
        conn.close()
        return rows
    # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!Frontend!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    class HouseView:
        ...
        def __init__(self):
            ...
            # ========================================Title==================================
            self.lblTitle = Label(TitleFrame, font=('arial', 56, 'bold'), text="Γενική εικόνα", bd=7)
            self.lblTitle.grid(row=0, column=0, padx=132)
            # ========================================TreeView==================================
            scroll_x = Scrollbar(RightFrame1a, orient=HORIZONTAL)
            scroll_y = Scrollbar(RightFrame1a, orient=VERTICAL)

            self.productlist = ttk.Treeview(RightFrame1a, height=9, columns=("name", "price"),
                                            xscrollcommand=scroll_x.set, yscrollcommand=scroll_y.set)
            ...
            self.productlist.insert(parent='', index=END, iid=0,
                                    values=("Σύνολο Μηνιαίων Εξόδων", viewSumExpenses()))
            self.productlist.insert(parent='', index=END, iid=0,
                                    values=("Σύνολο Supermarket", viewSumSupermarket()))
            self.productlist.insert(parent='', index=END, iid=0,
                                    values=("Σύνολο Ζημιών", viewSumZimies()))
            self.productlist.insert(parent='', index=END, iid=0,
                                    values=("Σύνολο εξόδων παιδιών", viewSumPaidia()))
            self.productlist.insert(parent='', index=END, iid=0,
                                    values=("Σύνολο Αγορών", viewSumAgores()))

            self.productlist.insert(parent='', index=END, iid=1,
                                    values=("Σύνολο Εσόδων", viewSumSalary()))
            self.productlist.configure()
            self.btnExit = Button(TopFrame1, pady=1, bd=4, font=('arial', 20, 'bold'), text="Έξοδος", padx=24, width=12,
                                  height=2, command=self.root.destroy).grid(row=0, column=4, padx=1)
...
companyIcon = PhotoImage(file='C:\\Users\\paulo\\Desktop\\company.png')
homeIcon = PhotoImage(file='C:\\Users\\paulo\\Desktop\\home.png')
brandIcon = PhotoImage(file='C:\\Users\\paulo\\Desktop\\aurum.png')
